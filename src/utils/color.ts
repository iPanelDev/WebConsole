    ...
    for (var i = 0; i < group.length; i++) {
        ...
        for (var arg_index = 0; arg_index < arg_group.length; arg_index++) {
            ...
            if (child_arg == '1') {
                ...
                styles.push('color: rgb(' + arg_group[arg_index + 2] + ',' + arg_group[arg_index + 3] + ',' + arg_group[arg_index + 4] + ')');
            }
            else if (child_arg == '48' && arg_group[arg_index + 1] == '2' && arg_index + 4 <= arg_group.length) {
                styles.push('background-color: rgb(' + arg_group[arg_index + 2] + ',' + arg_group[arg_index + 3] + ',' + arg_group[arg_index + 4] + ')');
            }
            else if (color_nums.indexOf(child_arg) >= 0) {
                classes.push('ANSI-' + child_arg);
            }
        }
        outputs.push(`<span style="${styles.join(';')}" class="${classes.join(' ')}">${match[2]}</span>`);
    }
    return outputs.join('');
}
