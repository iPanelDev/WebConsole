...
export const outputsMap = new Map<string, string[]>(JSON.parse(localStorage.getItem('ipanel.outputHistory')));
...
export function input(line: string) {
    ...
    localStorage.setItem('ipanel.inputHistory', JSON.stringify(inputHistory.value));
}
...
export function clearInvalidOutputHistory() {
    ...
    const invalid = Array.from(outputsMap.keys()).filter(key => !currentKeys.includes(key));
